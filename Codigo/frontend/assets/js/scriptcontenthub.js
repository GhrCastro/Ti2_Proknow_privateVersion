...
const releaseDOM = string => {
    ...
    return parser.parseFromString(sanitizedString, 'text/html').body.firstChild;
};
...
const alternateVideo = async target => {
    ...
    const sanitizedComments = DOMPurify.sanitize(cardComments(commentsFiltered));
    divCardComments.appendChild(releaseDOM(sanitizedComments));
...
divCardVideos.appendChild(releaseDOM(DOMPurify.sanitize(cards)));
divCardComments.appendChild(releaseDOM(DOMPurify.sanitize(comments)));
...
btnSendComment.addEventListener('click', async e => {
    ...
    const sanitizedData = DOMPurify.sanitize(cardComments(data));
    divCardComments.appendChild(releaseDOM(sanitizedData));
